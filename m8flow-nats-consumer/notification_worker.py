...
def _notify_one(tenant_id: str, reference_id: str) -> str:
    ...
    ``g.db_session`` for the call -- a request context (without request hooks) is what
    makes ``db.session`` resolve to it -- and committed or rolled back here."""
    from flask import g
    ...
    with flask_app.test_request_context("/internal/nats/notify"):
