...
from m8flow_backend.services import nats_monitoring_service as module
from m8flow_backend.services.nats_monitoring_service import (
...
def _response(url: str, status: int, *, json_body=None, text: str | None = None) -> requests.Response:
    response = requests.Response()
    response.status_code = status
    response.url = url
    response._content = (json.dumps(json_body) if text is None else text).encode("utf-8")
    return response
...
class TestNormalizeJsz:
    ...
    def test_passes_through_stream_config_and_message_timestamps(self, jsz):
        stream = NatsMonitoringService.normalize_jsz(jsz)["streams"][0]
        raw = jsz["account_details"][0]["stream_detail"][0]

        assert stream["firstTs"] == raw["state"]["first_ts"]
        assert stream["lastTs"] == raw["state"]["last_ts"]
        assert stream["config"] == {
            ...
            "maxConsumers": -1,
            "discard": "old",
            # 120s, reported by NATS in nanoseconds.
...
class TestBrokerFailuresBecome503:
    ...
    def test_timeout(self, enabled, monkeypatch):
        def _timeout(*_a, **_k):
            raise requests.ConnectTimeout("too slow")

        monkeypatch.setattr(module.requests, "get", _timeout)
    ...
    def test_connection_refused(self, enabled, monkeypatch):
        ...
        monkeypatch.setattr(module.requests, "get", _refused)

        with pytest.raises(ApiError) as exc:
            NatsMonitoringService.streams()
        assert exc.value.status_code == 503

    def test_unreachable_server_message_is_readable_not_a_stack_trace(self, enabled, monkeypatch):
        """The raw requests error ("HTTPConnectionPool(host='nats', ...) NameResolutionError")
        names internal hosts; the UI gets a plain explanation and the detail stays in logs."""

        def _dns_failure(*_a, **_k):
            raise requests.ConnectionError(
                "HTTPConnectionPool(host='nats', port=8222): Max retries exceeded "
                "(Caused by NameResolutionError(\"Failed to resolve 'nats'\"))"
            )

        monkeypatch.setattr(module.requests, "get", _dns_failure)

        with pytest.raises(ApiError) as exc:
            NatsMonitoringService.overview()
        assert exc.value.error_code == "nats_monitoring_unavailable"
        assert exc.value.message.startswith("The NATS server is not reachable.")
        assert "HTTPConnectionPool" not in exc.value.message
        assert "host=" not in exc.value.message

    def test_http_error_status(self, enabled, monkeypatch):
        def _server_error(url, **_k):
            return _response(url, 500, json_body={})

        monkeypatch.setattr(module.requests, "get", _server_error)
    ...
    def test_non_json_body(self, enabled, monkeypatch):
        ...
        def _html(url, **_k):
            return _response(url, 200, text="<html>nope</html>")

        monkeypatch.setattr(module.requests, "get", _html)
...
class TestFetchesRealEndpoints:
    def test_streams_asks_for_consumers_and_config(self, enabled, monkeypatch, jsz):
        ...
        def _capture(url, **_k):
            seen.append(url)
            return _response(url, 200, json_body=jsz)

        monkeypatch.setattr(module.requests, "get", _capture)
        NatsMonitoringService.streams()
    ...
    def test_overview_still_renders_when_healthz_fails(self, enabled, monkeypatch, varz, jsz):
        ...
        def _route(url, **_k):
            if "/healthz" in url:
                return _response(url, 503, json_body={})
            if "/jsz" in url:
                return _response(url, 200, json_body=jsz)
            return _response(url, 200, json_body=varz)

        monkeypatch.setattr(module.requests, "get", _route)
...
class TestSerializeMessage:
    ...
    def test_redacts_the_publisher_api_key(self, monkeypatch):
        monkeypatch.setattr(module, "nats_message_preview_max_bytes", lambda: 4096)
        body = b'{"id": "e1", "api_key": "m8f_secret.value", "payload": {"Token": "t\\"x"}}'

        result = NatsMonitoringService._serialize_message(_RawMessage(body), 7)

        assert "m8f_secret" not in result["payload"]
    ...
    def test_redacts_an_api_key_cut_off_by_the_preview_cap(self, monkeypatch):
        ...
        result = NatsMonitoringService._serialize_message(
            _RawMessage(b'{"api_key": "m8f_secret.value.long"}'), 7
        )
